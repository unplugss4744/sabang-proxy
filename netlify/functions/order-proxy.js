...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      // 1. Apps Script에 XML 저장 (PENDING_XML)
      const storeRes = await fetch(GAS_URL, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ secret:'0426', action:'storeOrderXml', xml })
      });

      // 2. Apps Script GET URL을 사방넷에 전달
      ...
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ status:'ok', orders, count:orders.length, raw:resultText.slice(0,300) })
      };
    } catch(err) {
...
function parseOrderXml(xmlStr) {
  ...
  try {
    ...
    while ((match = dataRegex.exec(xmlStr)) !== null) {
      ...
      orders.push({
        orderNo:g('IDX'), mallOrderNo:g('ORDER_ID'),
        receiver:g('RECEIVE_NAME'), phone1:g('RECEIVE_TEL'), phone2:g('RECEIVE_CEL'),
        zipCode:g('RECEIVE_ZIPCODE'), address:g('RECEIVE_ADDR'), addrDetail:'',
        personalNo:g('ORDER_ETC_14'), memo:g('DELV_MSG1'),
        productName:g('P_PRODUCT_NAME'), collectName:g('PRODUCT_NAME'),
        optionName:g('SKU_VALUE'), qty:g('SALE_CNT'),
        orderDate:g('ORDER_DATE'), orderStatus:g('ORDER_STATUS')
      });
    }
